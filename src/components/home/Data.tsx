import React, { useEffect, useRef, useState } from "react";
import gsap from "gsap";

const Data = () => {
  const dataRef = useRef<HTMLDivElement>(null);

  const [showToast, setShowToast] = useState(false);

  useEffect(() => {
    if (!dataRef.current) return;

    const title = dataRef.current.querySelector(".home__title");
    const subtitle = dataRef.current.querySelector(".home__subtitle");
    const description = dataRef.current.querySelector(".home__description");
    const buttons = dataRef.current.querySelectorAll(".button");

    gsap.fromTo(
      title,
      { opacity: 0, y: -40 },
      { opacity: 1, y: 0, duration: 2, ease: "power2.out" }
    );

    gsap.fromTo(
      subtitle,
      { opacity: 0, y: -30 },
      { opacity: 1, y: 0, duration: 2, delay: 0.2, ease: "power2.out" }
    );

    gsap.fromTo(
      description,
      { opacity: 0, y: 20 },
      { opacity: 1, y: 0, duration: 2, delay: 0.4, ease: "power2.out" }
    );

    gsap.fromTo(
      buttons,
      { opacity: 0, y: 20, scale: 0.9 },
      {
        opacity: 1,
        y: 0,
        scale: 1,
        duration: 2,
        delay: 0.6,
        stagger: 0.1,
        ease: "back.out(1.7)",
      }
    );
  }, []);

  useEffect(() => {
    if (!showToast) return;

    const timer = setTimeout(() => {
      setShowToast(false);
    }, 3500);

    return () => clearTimeout(timer);
  }, [showToast]);

  const handleResumeClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    setShowToast(true);
  };

  return (
    <div className="home__data" ref={dataRef}>
      <h1 className="home__title">
        <span className="logo-light">Tshimangadzo</span>
        <br />
        <span className="logo-dark">Munzhelele</span>
      </h1>

      <h3 className="home__subtitle">Software Developer</h3>

      <p className="home__description">
        Adaptable, resilient, and creative software developer driven by a
        passion for continuous learning and innovation, with a strong focus on
        usability and scalablity of solutions.
      </p>

      <div className="home__buttons">
        <a href="#contact" className="button button--flex">
          Contact
          <svg
            className="button__icon"
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
          >
            <path
              d="M14.2199 21.9352C13.0399 21.9352 11.3699 21.1052 10.0499 17.1352L9.32988 14.9752L7.16988 14.2552C3.20988 12.9352 2.37988 11.2652 2.37988 10.0852C2.37988 8.91525 3.20988 7.23525 7.16988 5.90525L15.6599 3.07525C17.7799 2.36525 19.5499 2.57525 20.6399 3.65525C21.7299 4.73525 21.9399 6.51525 21.2299 8.63525L18.3999 17.1252C17.0699 21.1052 15.3999 21.9352 14.2199 21.9352ZM7.63988 7.33525C4.85988 8.26525 3.86988 9.36525 3.86988 10.0852C3.86988 10.8052 4.85988 11.9052 7.63988 12.8252L10.1599 13.6652C10.3799 13.7352 10.5599 13.9152 10.6299 14.1352L11.4699 16.6552C12.3899 19.4352 13.4999 20.4252 14.2199 20.4252C14.9399 20.4252 16.0399 19.4352 16.9699 16.6552L19.7999 8.16525C20.3099 6.62525 20.2199 5.36525 19.5699 4.71525C18.9199 4.06525 17.6599 3.98525 16.1299 4.49525L7.63988 7.33525Z"
              fill="var(--container-color)"
            />
            <path
              d="M10.11 14.7052C9.92005 14.7052 9.73005 14.6352 9.58005 14.4852C9.29005 14.1952 9.29005 13.7152 9.58005 13.4252L13.16 9.83518C13.45 9.54518 13.93 9.54518 14.22 9.83518C14.51 10.1252 14.51 10.6052 14.22 10.8952L10.64 14.4852C10.5 14.6352 10.3 14.7052 10.11 14.7052Z"
              fill="var(--container-color)"
            />
          </svg>
        </a>

        <a
          href="#"
          className="button button--flex button--outline"
          onClick={handleResumeClick}
        >
          Resume
          <svg
            className="button__icon"
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
          >
            <path
              d="M12 16L7 11L8.4 9.55L11 12.15V4H13V12.15L15.6 9.55L17 11L12 16Z"
              fill="currentColor"
            />
            <path
              d="M5 20C4.45 20 3.979 19.804 3.587 19.412C3.195 19.02 2.99934 18.5493 3 18V15H5V18H19V15H21V18C21 18.55 20.804 19.021 20.412 19.413C20.02 19.805 19.5493 20.0007 19 20H5Z"
              fill="currentColor"
            />
          </svg>
        </a>
      </div>

      {/* ✅ TOAST */}
      <div className={`cv-toast ${showToast ? "show" : ""}`}>
        <p className="cv-toast__text">
          CV removed for now, Please contact me for the latest copy.
        </p>
        <button
          className="cv-toast__close"
          onClick={() => setShowToast(false)}
          aria-label="Close toast"
        >
          ✕
        </button>
      </div>
    </div>
  );
};

export default Data;
